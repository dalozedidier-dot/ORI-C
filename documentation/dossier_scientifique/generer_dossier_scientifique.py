...
add_para('Aucune fiche d’admission confirmatoire n’est actuellement présente. Les analyses sont donc publiées séparément comme preuves partielles. Pour IODP, la cohérence trace-réponse, la résistance à 20 mT et la comparaison trace naturelle/IRM-ARM ne satisfont pas les plans de C01, C02 et C04. La démagnétisation fournit une preuve forte d’ablation physique, mais pas le plan A/B complet de C03, qui reste non testable avec ce jeu. Les corrélations de Spearman utilisent des rangs moyens et la matrice permute uniquement au sein des strates expérimentales.')
