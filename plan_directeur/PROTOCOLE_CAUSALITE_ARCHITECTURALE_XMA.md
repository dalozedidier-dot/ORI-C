...
La prochaine instanciation astronomique distingue désormais deux niveaux. Premièrement, la spécificité de l'architecture des géantes reste à tester : Uranus et Neptune sont déjà dans le témoin, mais il faut intervenir sur elles avec des perturbations comparables à celles appliquées à Jupiter/Saturne et comparer les déplacements des modes `g_i/s_i` et la réponse terrestre. Deuxièmement, le spin terrestre a maintenant une **première instanciation exécutée au niveau modèle** dans `02_branche_systeme_solaire/couche_spin_orbite/` : le plan orbital N-corps force un axe de spin dynamique, le témoin avec couple lunaire effectif est comparé à La2004, l'ablation du couple lunaire est calculée et les interventions Jupiter/Saturne sont propagées jusqu'à l'obliquité et à l'insolation. Cette première exécution n'était pas préenregistrée ; elle reste exploratoire. Le prochain saut de preuve exige une réplication indépendante sous critères gelés, puis, pour traiter l'évolution tidale, une Terre-Lune explicitement résolue avec marées.
