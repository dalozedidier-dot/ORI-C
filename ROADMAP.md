...
| 2 | `H052 / HC01 / HC02` — fermeture matière | baseline scellé **46/53** ; extension `HC02-E1` **53/53**, matrice sémantique 4/4 | faire relire indépendamment la qualification source→sémantique ; toute objection fait retomber l’extension en fail-closed sans toucher au baseline | consolider la fermeture structurelle matière comme extension empirique auditable |
| 3 | `PRED-VIVANT-HISTOIRE-001` | prédiction gelée, aucun résultat prospectif ; deux routes LTEE installées | route A : réplication stricte avec nouvelle propagation + nouvelles MIC ; route B : généralisation prospective rapide sur matériel archivé, sans la surclasser en réplication | §XIV-3/4 ; route A vise aussi §XIV-10 |
| 4 | `MAG-PAIR-001` | protocole + pipeline + pilote technique installés ; `PACC-MAG-INT-01` prépare le même `PACC-INT-CHALLENGE-V1` que VES | confirmer le laboratoire, exécuter le pilote sacrificiel, figer champs/seuils/clé aveugle, puis préenregistrer les deux analyses avant toute donnée confirmatoire | §XIV-9 matière, §XIV-11 avec VES, et `PRED-MATIERE-ABLATION-001` |

## H052 : décision fail-closed

La fermeture 46/53 n'est pas un manque diffus de documentation. Le graphe est bloqué par une boucle de quatre nœuds. Deux réparations minimales ferment 53/53 en sensibilité. `HC01` recode H052. **`HC02` est désormais la voie prioritaire** : `N051 + N028 → N030`, puis H052 reste strictement canonique. Hao & Li 2018 et Ueda et al. 2021 donnent un support expérimental direct pour l’interface et sa chimie. Zhong et al. 2026 ferme la composante « catalyse »; HC02-E1 est désormais qualifiée en extension et atteint 53/53, tandis que le baseline gelé reste 46/53.
