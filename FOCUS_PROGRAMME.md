...
|---|---|---|
| D’Onofrio et prédiction antibiotique prospective | Le jeu D’Onofrio fournit un **benchmark externe rétrospectif positif** : RMSE 1,1309 pour l’état seul, 0,8042 avec l’histoire, témoin d’histoire permutée 1,1415, p = 0,00498. Il ne ferme pas la condition §XIV-3 car les données étaient déjà publiées et analysées. Le benchmark longitudinal amikacine est un protocole distinct : son gain face au témoin de complexité égale n’est pas stable (sign-flip p = 0,2266 ; ablation de pente p = 0,0078 ; nul ordonné p = 0,0649). Ces valeurs ne sont pas des tests de robustesse de D’Onofrio. | conserver `PRED-VIVANT-HISTOIRE-001` gelé et l’ouvrir uniquement sur un jeu indépendant ; succès seulement hors échantillon et contre témoin apparié |
...
| Mesure commune de `Pacc` | Plusieurs mesures locales existent : modèle astronomique/exoplanétaire, seuils `26Al`, support MIC antibiotique et support discret des descendants de vésicules. **Aucune ne ferme la condition empirique §XIV-9 dans les trois branches.** Le proxy observationnel historique à cinq classes est saturé (`Pacc = 1`) ; le support vésiculaire est rétrospectif ; EXO-DOM-01 est causal mais au niveau modèle. | appliquer la définition stricte `PACC-INT-CHALLENGE-V1` : défis et seuils gelés, `X/Theta/A` appariés, intervention ciblée sur `m`, sham et unités indépendantes. Un proxy observationnel ne peut plus être promu en `Pacc` causal |
