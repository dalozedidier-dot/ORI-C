...
|---|---|---|
| Prédiction antibiotique hors échantillon | **Premier résultat positif obtenu** sur le jeu D’Onofrio : RMSE 1,1309 pour l’état seul, 0,8042 avec l’histoire, témoin permuté 1,1415, p = 0,00498. Une extension exploratoire mesure aussi un support `P_acc` rétrospectif moyen de 0,4993 contre 0,5758 sous histoire permutée de même complexité, p = 0,00100 | réplication sur un autre jeu indépendant avec le même protocole gelé, puis isolement d’une trace biologique `m` physiquement mesurée avant toute revendication mécanistique |
...
| Mesure commune de `Pacc` | **plusieurs mesures locales existent désormais** : astronomique au niveau modèle, seuils `26Al`, support MIC antibiotique et support discret des descendants de vésicules. Elles ne partagent pas encore une échelle commune ; le nouveau contraste `P_acc` sous ablation vésiculaire est négatif pour la direction attendue (−0,0375) | construire une métrique inter-domaines explicitement comparable avant tout test d’invariant ; conserver séparément les proxies rétrospectifs, les seuils physiques et les domaines d’intervention |
