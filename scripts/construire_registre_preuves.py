...
if gc_path.is_file():
 ...
 if gc.get('schema')!='oric.gc.claims': raise ValueError('schéma CLAIMS généalogie cosmique inattendu')
 for c in gc['claims']:
...
# Généalogie cosmique quantitative complète : huit résultats physiques/audits
...
if gcq_path.is_file():
 ...
 if gcq.get('schema')!='oric.gc.quantitative-claims-complete': raise ValueError('schéma claims quantitatifs complets inattendu')
 for c in gcq['claims']:
  cid=c['claim_id']; artefact=f'01_branche_matiere/genealogie_cosmique_quantitative/resultats/claims_quantitatifs/{cid}.json'
...
# Généalogie cosmique quantitative sur distributions massives réelles, sans double comptage.
...
if gcq_massive_path.is_file():
 ...
 if gcq_massive.get('schema')!='oric.gc.quantitative-massive-data-claims': raise ValueError('schéma claims quantitatifs données massives inattendu')
 for c in gcq_massive['claims']:
  cid=c['claim_id']; artefact=f'01_branche_matiere/genealogie_cosmique_quantitative/resultats/claims_quantitatifs/{cid}.json'
  entries.append({'id':cid,'question':c['question'],'statut':'extension_quantitative_empirique_donnees_massives_non_preregistered','verdict':c['verdict'],'niveau_preuve':None,'portee':'distributions réelles grain/échantillon — sans simulation/synthétique/imputation ni double comptage de formats','artefact':artefact,'empreinte_sortie':sha(artefact),'source':c.get('source_ids'),'mesures':{'stages':c.get('stage_ids'),'criterion_met':c.get('criterion_met'),'preregistered':c.get('preregistered'),'data_policy':c.get('data_policy')},'supersede_par':None})
