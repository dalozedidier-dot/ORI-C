...
# Généalogie cosmique quantitative v4 : distributions massives réelles, sans double comptage.
gcq4_path=ROOT/'01_branche_matiere/genealogie_cosmique_quantitative/resultats/CLAIMS_QUANTITATIFS_DATA_RICH_V4.json'
if gcq4_path.is_file():
 gcq4=json.loads(gcq4_path.read_text(encoding='utf-8'))
 if gcq4.get('schema')!='oric.gc.quantitative-claims.v4': raise ValueError('schéma claims quantitatifs v4 inattendu')
 for c in gcq4['claims']:
  cid=c['claim_id']; artefact=f'01_branche_matiere/genealogie_cosmique_quantitative/resultats/claims_quantitatifs_v4/{cid}.json'
  entries.append({'id':cid,'question':c['question'],'statut':'extension_quantitative_empirique_data_rich_non_preregistered','verdict':c['verdict'],'niveau_preuve':None,'portee':'distributions réelles grain/échantillon — sans simulation/synthétique/imputation ni double comptage de formats','artefact':artefact,'empreinte_sortie':sha(artefact),'source':c.get('source_ids'),'mesures':{'stages':c.get('stage_ids'),'criterion_met':c.get('criterion_met'),'preregistered':c.get('preregistered'),'data_policy':c.get('data_policy')},'supersede_par':None})
