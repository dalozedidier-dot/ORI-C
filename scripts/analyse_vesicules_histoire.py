...
DATA=ROOT/'data'; OUT=ROOT/'resultats'/'VESICULES_HISTOIRE_CONTEXTE.json'
SEED=20260818
...
existing=json.loads((ROOT/'source_snapshots'/'VESICULES_DYNAMIQUE_TEMPORELLE_v1.json').read_text())
result={
 ...
 'inputs':{'prebiotic_lineages.csv':{'rows':len(lin),'sha256':sha(DATA/'prebiotic_lineages.csv')},
           'prebiotic_timecourses.csv':{'rows':existing['rows'],'sha256':sha(DATA/'prebiotic_timecourses.csv')}},
