...
if version!='0.9.4-research': errors.append(f'VERSION inattendue: {version}')
for p in ['CITATION.cff','.zenodo.json','LICENSE','LICENSING.md','RELEASE_NOTES_v0.9.4-research.md','PUBLICATION_STABLE.md','.github/workflows/release.yml']:
    if not (ROOT/p).exists(): errors.append(f'manquant: {p}')
if version not in (ROOT/'CITATION.cff').read_text(): errors.append('CITATION.cff non synchronisé')
...
for needle in ['7,02 Ma','1 critère sur 10','tolérance','3 fenêtres sur 3','Trajectoires réelles','31 nœuds','H011','MESA']:
