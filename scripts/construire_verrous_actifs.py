    ...
    data={
      ...
      'fronts':[
        {'rank':1,'id':'VES-PACC-INT-01','branch':'vivant','scientific_design_complete':True,
         'public_registration_status':vesreg.get('status'),'public_url_present':bool(vesreg.get('public_url')),'execution_package_status':vesexec.get('status'),'candidate_lab_count':len(veslabs.get('candidates',[])),
        ...
        {'rank':2,'id':'H052-HC01-HC02','branch':'matiere','canonical_closure':f"{h['canonical_closure']['reachable_nodes']}/{h['canonical_closure']['total_nodes']}",
         'candidate_status':h['candidate_hc01_r1']['status'],'hc02_status':h.get('candidate_hc02_direct_interface',{}).get('status'),'hc02_preferred_next_audit':h.get('candidate_hc02_direct_interface',{}).get('preferred_over_hc01_for_next_audit',False),'verdict':h['verdict'],'extension_closure':'53/53' if h.get('hc02_extension',{}).get('status')=='evidence_qualified_extension' else None,'targets':['fermeture_hypergraphe_53_53']},
        {'rank':3,'id':'PRED-VIVANT-HISTOIRE-001','branch':'vivant','status':predv['statut'],'data_opened':predv.get('date_ouverture') is not None,
         'result_present':predv.get('resultat') is not None,'new_MIC_route_status':repplan.get('status'),'targets':['XIV-3','XIV-4','XIV-10']},
        {'rank':4,'id':'MAG-PAIR-001','branch':'matiere','status':mag['status'],'minimum_independent_units':mag['minimum_independent_units'],'execution_freeze_status':magexec.get('status'),'remaining_lab_fields':sum(v is None for v in magexec.get('frozen_fields',{}).values()),'pilot_installed':True,
         'parallel_pacc_design':'PACC-MAG-INT-01','targets':['XIV-9','XIV-11','PRED-MATIERE-ABLATION-001']},
      ]
    }
