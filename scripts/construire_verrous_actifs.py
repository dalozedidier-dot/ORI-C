    ...
    data={
      ...
      'fronts':[
        {'rank':1,'id':'VES-PACC-INT-01','branch':'vivant','scientific_design_complete':True,
         'public_registration_status':vesreg.get('status'),'public_url_present':bool(vesreg.get('public_url')),'execution_package_status':vesexec.get('status'),'candidate_lab_count':len(vesexec.get('candidate_labs',[])),
        ...
        {'rank':2,'id':'H052-HC01-HC02','branch':'matiere','canonical_closure':f"{h['canonical_closure']['reachable_nodes']}/{h['canonical_closure']['total_nodes']}",
         'candidate_status':h['candidate_hc01_r1']['status'],'hc02_status':h.get('candidate_hc02_direct_interface',{}).get('status'),'hc02_preferred_next_audit':h.get('candidate_hc02_direct_interface',{}).get('preferred_over_hc01_for_next_audit',False),'verdict':h['verdict'],'targets':['fermeture_hypergraphe_53_53']},
